...
void desenhar_linha_conectando_dois_pontos(float* pos_km_x,
                                           float* pos_km_y,
                                           int screen_width,
                                           int screen_height);
...
public:
          ...
          m_w(1280), m_h(720)
    {
        ...
        {
            wxLogWarning(
                "Fonte não encontrada. Usando fonte padrão.");
...
private:
    ...
    void handleStorePoint()
    {
        ...
        {
            armazenar_posicao_mouse(m_mouse_x,
                                    m_mouse_y,
                                    m_w,
                                    m_h,
                                    m_pos_km_x,
    ...
    void drawScene()
    {
        al_set_target_bitmap(m_buffer);
        al_clear_to_color(
            al_map_rgb(0, 0, 0));  // fundo preto por frame
        ...
        for (int i = 0; i < m_points_stored; i++)
        {
            ...
            converter_km_para_pixel(
                m_pos_km_x[i], m_pos_km_y[i], m_w, m_h, &px, &py);
            al_draw_filled_circle(px, py, 10, al_map_rgb(0, 0, 255));
            snprintf(buffer,
                     sizeof(buffer),
        ...
        if (m_points_stored == 2)
        {
            ...
            snprintf(buffer,
                     sizeof(buffer),
                     "Módulo: u=%.2f km",
                     calcular_modulo_vetor(m_pos_km_x[0],
                                           m_pos_km_y[0],
                                           m_pos_km_x[1],
                                           m_pos_km_y[1]));
            al_draw_text(m_font,
    ...
    void OnPaint(wxPaintEvent&)
    {
        ...
        const unsigned char* src =
            static_cast<const unsigned char*>(lr->data);
