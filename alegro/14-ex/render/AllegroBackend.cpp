...
AllegroBackend::~AllegroBackend() { shutdown(); }
...
void AllegroBackend::render(const core::RenderModel &model) {
  ...
  for (int i = 0; i < model.points_stored; ++i) {
    ...
    if (font_)
      al_draw_text(font_, al_map_rgb(255, 255, 255), 20, 20 + i * 30, 0,
                   buffer);
  }

  if (model.points_stored == 2) {
    float x1, y1, x2, y2;
    core::converter_km_para_pixel(model.pos_km_x[0], model.pos_km_y[0],
                                  model.width, model.height, &x1, &y1);
    core::converter_km_para_pixel(model.pos_km_x[1], model.pos_km_y[1],
                                  model.width, model.height, &x2, &y2);
    ...
    if (font_)
      al_draw_text(font_, al_map_rgb(255, 255, 255), 20, 80, 0, buffer);

    std::snprintf(
        buffer, sizeof(buffer), "Módulo: u=%.2f km",
        core::calcular_modulo_vetor(model.pos_km_x[0], model.pos_km_y[0],
                                    model.pos_km_x[1], model.pos_km_y[1]));
    if (font_)
      al_draw_text(font_, al_map_rgb(255, 255, 255), 20, 110, 0, buffer);
  }
...
bool AllegroBackend::getFrameRGB(std::vector<unsigned char> &outRGB, int &width,
                                 int &height) {
  if (!initialized_ || !backbuffer_)
    return false;
