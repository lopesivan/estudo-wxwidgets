...
bool AllegroBackend::init(void* /*native_handle*/, int width, int height) {
...
void AllegroBackend::render(const core::RenderModel& model) {
    if (!initialized_ || !backbuffer_) return;
    ...
    for (int i = 0; i < model.points_stored; ++i) {
        float px, py;
        core::converter_km_para_pixel(model.pos_km_x[i], model.pos_km_y[i],
                                      model.width, model.height, &px, &py);
        al_draw_filled_circle(px, py, 10, al_map_rgb(255, 0, 0));
        std::snprintf(buffer, sizeof(buffer),
                      "Posição %d: X=%.2f km, Y=%.2f km",
                      i + 1, model.pos_km_x[i], model.pos_km_y[i]);
        if (font_) al_draw_text(font_, al_map_rgb(255,255,255), 20, 20 + i*30, 0, buffer);
    }

    if (model.points_stored == 2) {
        float x1,y1,x2,y2;
        core::converter_km_para_pixel(model.pos_km_x[0], model.pos_km_y[0], model.width, model.height, &x1, &y1);
        core::converter_km_para_pixel(model.pos_km_x[1], model.pos_km_y[1], model.width, model.height, &x2, &y2);
        ...
        if (font_) al_draw_text(font_, al_map_rgb(255,255,255), 20, 80, 0, buffer);

        std::snprintf(buffer, sizeof(buffer),
                      "Módulo: u=%.2f km",
                      core::calcular_modulo_vetor(model.pos_km_x[0], model.pos_km_y[0],
                                                  model.pos_km_x[1], model.pos_km_y[1]));
        if (font_) al_draw_text(font_, al_map_rgb(255,255,255), 20, 110, 0, buffer);
    }

    if (font_) {
        al_draw_text(font_, al_map_rgb(200,200,200), 20, model.height - 40, 0,
...
bool AllegroBackend::getFrameRGB(std::vector<unsigned char>& outRGB,
                                 ...
                                 int& height)
{
    if (!initialized_ || !backbuffer_) return false;

    width = w_;
    height = h_;
    outRGB.resize(w_ * h_ * 3);

    // Leitura neutra a formato (sempre correta): al_get_pixel + al_unmap_rgba
    // (Suficiente para 1280x720. Se quiser mais desempenho, podemos trocar
    //  por lock + conversão conforme al_get_bitmap_format().)
    ALLEGRO_LOCKED_REGION* lr = al_lock_bitmap(backbuffer_, 0, ALLEGRO_LOCK_READONLY);
    ...
    if (lr) al_unlock_bitmap(backbuffer_);

    return true;
}

void AllegroBackend::shutdown() {
    if (font_) { al_destroy_font(font_); font_ = nullptr; }
    if (backbuffer_) { al_destroy_bitmap(backbuffer_); backbuffer_ = nullptr; }
    if (initialized_) {
