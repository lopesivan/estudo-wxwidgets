...
public:
    explicit RenderView(wxWindow* parent,
          ...
          backend_(std::move(backend))
    {
        ...
        Bind(wxEVT_ERASE_BACKGROUND, [](wxEraseEvent&){ /* evita clear do wx */ });
        ...
        backend_->init(/*native_handle*/nullptr, model_.width, model_.height);
    }
...
private:
    ...
    std::vector<unsigned char> frameRGB_; // buffer temporário para wxImage
    ...
    void OnCharHook(wxKeyEvent& e) {
        ...
        if (code == WXK_ESCAPE) {
            if (auto* tlw = wxGetTopLevelParent(this)) tlw->Close(false);
            return;
        } else if (code == WXK_SPACE) {
            if (model_.points_stored < 2) {
                core::armazenar_posicao_mouse(
                    model_.mouse_x_px, model_.mouse_y_px,
                    model_.width, model_.height,
                    model_.pos_km_x, model_.pos_km_y, model_.points_stored);
    ...
    void OnPaint(wxPaintEvent&) {
        ...
        int w=0, h=0;
        if (!backend_->getFrameRGB(frameRGB_, w, h)) return;

        wxImage img(w, h);
        std::memcpy(img.GetData(), frameRGB_.data(), frameRGB_.size());
        wxBitmap bmp(img);
