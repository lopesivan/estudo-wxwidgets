...
#define LARGURA 800
#define ALTURA 600
#define ESCALA 50.0f  // 50 pixels = 1 unidade no plano

typedef struct {
    float x, y;  // coordenadas no plano cartesiano
    float raio;
    bool selecionado;
} Circulo;
...
// Converte coordenadas do plano para pixels
void mundo_para_tela(float mx, float my, int *px, int *py) {
    *px = LARGURA / 2 + (int)(mx * ESCALA);
    *py = ALTURA / 2 - (int)(my * ESCALA);  // Y invertido
}

// Converte pixels para coordenadas do plano
...
int main() {
    ...
    while (!sair) {
        ...
        if (evento.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
            ...
            // Verifica se clicou em algum círculo
            ...
            // Se não clicou em nenhum, cria um novo
            if (circulo_selecionado == -1 && num_circulos < 100) {
                ...
                circulos[num_circulos].selecionado = false;
                num_circulos++;
            }
        }
