...
public:
    AptManagerFrame() : wxFrame(nullptr, wxID_ANY, "Gerenciador de Pacotes APT", 
...
private:
    ...
    void CreateControls() {
        ...
        btnListUpgradable = new wxButton(panel, wxID_ANY, "Pacotes Atualizáveis");
        ...
        // Lista de pacotes
        packageList = new wxListCtrl(panel, wxID_ANY, wxDefaultPosition, 
        ...
        outputText = new wxTextCtrl(panel, wxID_ANY, "", wxDefaultPosition, 
        ...
        mainSizer->Add(new wxStaticText(panel, wxID_ANY, "Saída:"), 
                       0, wxLEFT | wxTOP, 10);
        ...
        btnListUpgradable->Bind(wxEVT_BUTTON, &AptManagerFrame::OnListUpgradable, this);
    ...
    void LoadInstalledPackages() {
        ...
        wxString output = ExecuteCommand("dpkg-query -W -f='${Package}|${Version}|${Architecture}\\n'");
        ...
        for (size_t i = 0; i < lines.GetCount(); i++) {
            ...
            if (!line.IsEmpty()) {
                ...
                if (parts.GetCount() >= 3) {
                    long idx = packageList->InsertItem(packageList->GetItemCount(), parts[0]);
    ...
    void OnUpgrade(wxCommandEvent& event) {
        int answer = wxMessageBox("Deseja realmente atualizar os pacotes? "
