...
public:
  // imagePath: caminho do PNG (com alpha). Se não encontrar, faz fallback no
  // círculo sólido.
  explicit BitmapPainter(wxString imagePath, wxColour fallbackColor = *wxBLUE,
                         bool antialiasFallback = true)
      : m_imagePath(std::move(imagePath)), m_fallbackColor(fallbackColor),
...
public:
  static std::shared_ptr<IBallPainter> Make(PainterKind kind) {
    ...
    case PainterKind::Bitmap:
      ...
      return std::make_shared<BitmapPainter>(wxString("ball.png"), *wxBLUE,
...
public:
  explicit BouncingBallPanel(wxWindow *parent, int timerIntervalMs = 20)
      : wxPanel(parent, wxID_ANY), m_timer(this),
        m_timerIntervalMs(timerIntervalMs), m_activeKind(PainterKind::Circle),
        m_activePainter(PainterFactory::Make(m_activeKind)) {
    SetBackgroundStyle(wxBG_STYLE_PAINT);
    SetDoubleBuffered(true);

    // Uma bola inicial
    AddBall({50.0, 50.0, 160.0, 140.0, FromDIP(wxSize(20, 20)).GetWidth(),
             *wxBLUE});
  ...
  void SetActivePainter(PainterKind kind) {
    ...
    if (auto *frame = wxDynamicCast(GetParent(), wxFrame)) {
      wxString s = (kind == PainterKind::Circle) ? "Strategy: Circle"
                                                 : "Strategy: Bitmap";
  ...
  void UpdateStatus() {
    if (auto *frame = wxDynamicCast(GetParent(), wxFrame)) {
      frame->SetStatusText(wxString::Format(
          "Paused: %s | Speed: %.3fx | Balls: %zu", m_paused ? "Yes" : "No",
          m_speedScale, m_balls.size()));
    }
  }
...
public:
  explicit MyFrame(const wxString &title)
    ...
    CreateStatusBar();
    SetStatusText(
        "C: Circle | B: Bitmap | N: New ball | Space: Pause | +/-: Speed");
