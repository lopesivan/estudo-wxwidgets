...
SpinFrame::SpinFrame()
    : wxFrame(NULL, wxID_ANY, "Aplicativo com Botão Giratório"),
      m_counter(0)
{
    // Criar menu
    ...
    wxStaticText* title = new wxStaticText(
        panel, wxID_ANY, "Contador com Botão Giratório");
    ...
    // Botão giratório
    m_spinButton = new wxSpinButton(panel,
                                    ID_SpinButton,
                                    wxDefaultPosition,
                                    wxDefaultSize,
                                    wxSP_VERTICAL);
    ...
    wxStaticBoxSizer* spinCtrlBox = new wxStaticBoxSizer(
        wxVERTICAL, panel, "Controle de Spin com Texto");

    wxBoxSizer* spinCtrlSizer = new wxBoxSizer(wxHORIZONTAL);

    wxStaticText* spinCtrlLabel =
        new wxStaticText(panel, wxID_ANY, "Valor:");
    ...
    m_spinCtrlText =
        new wxStaticText(panel, wxID_ANY, "Valor atual: 0");
    ...
    wxStaticText* instructions =
        new wxStaticText(panel,
                         wxID_ANY,
                         "Use as setas do botão giratório para "
                         "incrementar/decrementar o "
                         "contador.\n"
                         "O SpinCtrl permite digitar o valor "
                         "diretamente ou usar as setas.");
...
void SpinFrame::OnSpinUp(wxSpinEvent& event)
{
    ...
    SetStatusText(wxString::Format("Contador incrementado para: %d",
                                   m_counter));
}
...
void SpinFrame::OnSpinDown(wxSpinEvent& event)
{
    m_counter--;
    m_counterText->SetLabel(wxString::Format("%d", m_counter));
    SetStatusText(wxString::Format("Contador decrementado para: %d",
                                   m_counter));
}
...
void SpinFrame::OnSpinCtrl(wxSpinEvent& event)
{
    ...
    m_spinCtrlText->SetLabel(
        wxString::Format("Valor atual: %d", value));
    SetStatusText(
        wxString::Format("SpinCtrl alterado para: %d", value));
}
...
{
    wxMessageBox("Aplicativo demonstrando o uso de controles "
                 "giratórios em wxWidgets",
                 "Sobre",
