...
wxBEGIN_EVENT_TABLE(MyFrame, wxFrame) EVT_MENU(wxID_EXIT, MyFrame::OnExit)
    wxEND_EVENT_TABLE()
  ...
  int *gl_attrib = NULL;
#else
  int gl_attrib[20] = {WX_GL_RGBA, WX_GL_MIN_RED,      1, WX_GL_MIN_GREEN,
                       1,          WX_GL_MIN_BLUE,     1, WX_GL_DEPTH_SIZE,
                       1,          WX_GL_DOUBLEBUFFER, 0};
#endif
...
wxBEGIN_EVENT_TABLE(TestGLCanvas, wxGLCanvas) EVT_SIZE(TestGLCanvas::OnSize)
    EVT_PAINT(TestGLCanvas::OnPaint) EVT_CHAR(TestGLCanvas::OnChar)
        EVT_MOUSE_EVENTS(TestGLCanvas::OnMouseEvent) wxEND_EVENT_TABLE()
...
void TestGLCanvas::LoadSurface(const wxString &filename) {
...

  {
    ...
    {
      inFile >> m_verts[m_numverts][0] >> m_verts[m_numverts][1] >>
          m_verts[m_numverts][2];
      inFile >> m_norms[m_numverts][0] >> m_norms[m_numverts][1] >>
          m_norms[m_numverts][2];
  ...
  wxLogMessage("Loaded %d vertices, %d triangles from '%s'", m_numverts,
               m_numverts - 2, filename);

  // NOTE: for some reason under wxGTK the following is required to avoid that
  //       the surface gets rendered in a small rectangle in the top-left corner
  //       of the frame
