...
// Due to oglpfuncs.h needs to be included before gl.h (to avoid some
// declarations), we include glcanvas.h after oglstuff.h
#include "oglstuff.h"
#include "pyramid.h"
...
wxBEGIN_EVENT_TABLE(MyFrame, wxFrame) EVT_MENU(Pyramid_Quit, MyFrame::OnQuit)
...
void MyFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
  wxMessageBox(wxString::Format("Welcome to %s!\n"
                                ...
                                "%s\n",
                                wxVERSION_STRING, m_OGLString),
               "About wxWidgets pyramid sample", wxOK | wxICON_INFORMATION,
               this);
}
...
wxBEGIN_EVENT_TABLE(MyGLCanvas, wxGLCanvas) EVT_PAINT(MyGLCanvas::OnPaint)
    EVT_SIZE(MyGLCanvas::OnSize) EVT_MOUSE_EVENTS(MyGLCanvas::OnMouse)
        wxEND_EVENT_TABLE()
