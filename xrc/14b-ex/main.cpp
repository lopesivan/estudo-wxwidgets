...
public:
  bool OnInit() override {
    ...
    wiz->Bind(wxEVT_WIZARD_PAGE_CHANGED, [p3, c1, c2, c3](wxWizardEvent& e) {
      ...
      wxTextCtrl* userCtrl = dynamic_cast<wxTextCtrl*>(c2->FindWindow(XRCID("TEXT_USERNAME")));
      wxTextCtrl* mailCtrl = dynamic_cast<wxTextCtrl*>(c2->FindWindow(XRCID("TEXT_EMAIL")));
      wxCheckBox* termsChk = dynamic_cast<wxCheckBox*>(c1->FindWindow(XRCID("CHECK_TERMS")));
      wxStaticText* summary = dynamic_cast<wxStaticText*>(c3->FindWindow(XRCID("TEXT_SUMMARY")));
